...
namespace _06.SubsetsStrings
{
    public class Startup
    {
        ...
        public static void Main()
        {
            ...
            GenerateUniqueCombination(numbers, combinationLength, elementsCount);
        }
        ...
        private static void GenerateUniqueCombination(int[] range, int current, int count)
        {
            for (int i = 0; i < count; i++)
            {
                if (current > 1)
                {
                    GenerateUniqueCombination(range, current - 1, count);
                }
                else
                {
                    if (!HasDuplicateElements(range))
                    {
                        PrintElements(range);
                    }
                }
